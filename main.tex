...
\begin{document}
 ...
 متلب، یک محیط نرم‌افزاری برای انجام محاسبات عددی و یک زبان برنامه‌نویسی نسل چهارم است که از ترکیب دو واژهٔ MATrix (ماتریس) و LABoratory (آزمایشگاه) ایجاد شده‌است.  این نام حاکی از رویکرد ماتریس محور برنامه است، که در آن حتی اعداد منفرد هم به عنوان ماتریس در نظر گرفته می‌شوند.
    ...
    \begin{latin}
%    \lstinputlisting[numbers=left,frame=leftline,language=Octave]{simpleSignals.m}
    \end{latin}
    
    \begin{latin}
 %   	\lstinputlisting[numbers=left,frame=leftline,language=Mathematica]{mathe.nb}
    \end{latin}
    ...
    \item[ورودی]
    دستور ورودی به شکل \lr{\lstinline[language=Octave]{? variable}} است که در آن \lr{\lstinline[language=Octave]{variable}} نام یک متغیر است. اجرای این دستور موجب می‌شود که مفسر منتظر گرفتن یک عدد صحیح از ورودی بماند تا کاربر عدد را وارد کند.
    \item[خروجی]
    دستور خروجی به شکل \lr{\lstinline[language=Octave]{! variable}} است که در آن \lr{\lstinline[language=Octave]{variable}} نام یک متغیر است. اجرای این دستور موجب می‌شود که مفسر مقدار متغیر مربوطه را در یک خط مجزا در خروجی بنویسد.
    \item[جایگزینی]
    شکل این دستور \lr{\lstinline[language=Octave]{variable = expression}} است که با اجرای آن عبارت \lr{\lstinline[language=Octave]{expression}} ارزشیابی شده، مقدار آن در متغیر \lr{\lstinline[language=Octave]{variable}} گذاشته می‌شود.
    \end{description}
    ...
    بخش اول برنامه‌ی زبانچه است که در انتهای آن خطی وجود دارد که حاوی رشته‌ی \lr{\lstinline[language=Octave]{run}} است.
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|}
            ...
            %\lstinputlisting[numbers=left,language=Octave,basicstyle=\footnotesize\ttfamily\color{Gray}]{../testcases/1.in}
            ...
            \hline
            \begin{latin}\begin{minipage}[]{0.3\textwidth}\footnotesize
            %\lstinputlisting[numbers=left,language=Octave,basicstyle=\footnotesize\ttfamily\color{Gray}]{../testcases/2.in}
            \end{minipage}\end{latin}
            &
            \begin{latin}\begin{minipage}[]{0.3\textwidth}\footnotesize
            ...
            \hline
            \begin{latin}\begin{minipage}[]{0.3\textwidth}\footnotesize
            %\lstinputlisting[numbers=left,language=Octave,basicstyle=\footnotesize\ttfamily\color{Gray}]{../testcases/3.in}
            \end{minipage}\end{latin}
            &
            \begin{latin}\begin{minipage}[]{0.3\textwidth}\footnotesize
